...
JMeter বা Locust নিজে থেকেই percentile হিসাব করে দেয়, কিন্তু ধারণাটা পোক্ত করতে চলো নিজেরাই একটা ছোট Python স্ক্রিপ্ট দিয়ে দেখি কীভাবে কাজ করে — ধরে নিচ্ছি আমরা একটা লিস্টে রেসপন্স টাইম সংগ্রহ করেছি:
...
def calculate_percentile(times: list[float], percentile: float) -> float:
    ...
    index = math.ceil((percentile / 100) * len(sorted_times)) - 1
...
print("p50:", calculate_percentile(response_times, 50))  # typical
print("p95:", calculate_percentile(response_times, 95))  # 950ms আউটলায়ার ধরা পড়বে
print("p99:", calculate_percentile(response_times, 99))
```

এই স্ক্রিপ্ট চালালে দেখবে p50 প্রায় ১০০ms দেখাচ্ছে (স্বাভাবিক), কিন্তু p95 বা p99-এ গিয়ে সেই ৯৫০ms আউটলায়ারটা ধরা পড়ে যাচ্ছে — যেটা শুধু average নিলে চাপা পড়ে যেতো। প্রোডাকশন কোডে এই হিসাব হাতে না লিখে Python-এর built-in `statistics.quantiles()` বা NumPy-এর `numpy.percentile()` ব্যবহার করাই ভালো — এখানে হাতে-কলমে দেখানো হলো শুধু ধারণাটা স্পষ্ট করার জন্য।

**একটা কমন ভুল** — percentile হিসাব করার সময় ডেটাসেটের আকার (sample size) খুব ছোট হলে ফলাফল বিভ্রান্তিকর হতে পারে। যদি তুমি মাত্র ২০টা রিকোয়েস্টের উপর p99 হিসাব করো, তাহলে p99 মানে হলো "সবচেয়ে ধীর রিকোয়েস্টটা" — কারণ ২০-এর ৯৯% হলো প্রায় শেষ ইনডেক্স। p99 অর্থবহ হতে হলে অন্তত কয়েক হাজার স্যাম্পল লাগে (JMeter বা Locust-এ লোড টেস্টে এটা এমনিতেই হয়ে যায়), নাহলে সংখ্যাটা statistically noise-এর বেশি কিছু বলে না।
