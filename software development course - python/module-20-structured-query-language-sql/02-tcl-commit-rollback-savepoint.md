...
একটা গুরুত্বপূর্ণ বাস্তব নিয়ম — Python/FastAPI অ্যাপ্লিকেশনে Transaction ব্যবহার করার সময় সবসময় `try/except` দিয়ে wrap করা উচিত, যাতে কোনো এরর হলে অ্যাপ্লিকেশন কোড নিজে `ROLLBACK` চালাতে পারে:
...
try:
    ...
    conn.execute("INSERT INTO orders (customer_id) VALUES (%s)", (1,))
    conn.execute("COMMIT")
except Exception:
...
এই কোডটা ঠিক Module 19-এর প্রতিটা ডোমেইনে আমরা যে SQL ব্লক দেখিয়েছিলাম, সেটারই Python-সাইড বাস্তবায়ন। এখন Transaction-এর ভেতরের একক কমান্ডগুলোর দিকে ফিরি — বিশেষ করে যখন একাধিক টেবিল থেকে একসাথে ডেটা আনতে হয়, তখন লাগে JOIN, যেটা আমরা পরের লেসনে শিখবো।
