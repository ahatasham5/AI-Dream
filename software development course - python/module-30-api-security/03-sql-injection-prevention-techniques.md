...
username = request_body["username"]  # ধরি: admin' --
...
Python-এ raw SQL ব্যবহার করলে (যেমন `databases` লাইব্রেরি বা সরাসরি SQLAlchemy Core দিয়ে কাজ করার সময়), parameterized query দেখতে এমন:
...
এখানে `:username` একটা প্লেসহোল্ডার, আর আসল মানটা আলাদা dict-এ পাঠানো হচ্ছে। ড্রাইভার নিজে ভেতরে ভেতরে নিশ্চিত করে যে যতই দূষিত স্ট্রিং আসুক, সেটা শুধু "একটা তুলনার মান" হিসেবেই ব্যবহৃত হবে, SQL কাঠামোর অংশ হিসেবে না।

আধুনিক FastAPI প্রজেক্টে, আমরা প্রায়ই raw SQL-এর বদলে একটা ORM (Object-Relational Mapper) ব্যবহার করি — যেমন SQLAlchemy — যা এই parameterization ডিফল্টভাবে, স্বয়ংক্রিয়ভাবে করে দেয়:

```python
# SQLAlchemy ORM উদাহরণ — এখানে ORM নিজেই parameterization নিশ্চিত করছে
...
```python
# SQLAlchemy Query API উদাহরণ (পুরনো স্টাইল, তবু সমান নিরাপদ)
user = db.query(User).filter(User.username == username).first()
```

এই দুটো উদাহরণেই আমরা কখনো সরাসরি SQL স্ট্রিং লিখছি না — ORM নিজেই সঠিক parameterized query বানিয়ে ডেটাবেজে পাঠায়। এই কারণে বেশিরভাগ আধুনিক প্রজেক্টে ORM ব্যবহার SQL Injection-এর ঝুঁকি বহুলাংশে কমিয়ে দেয়। তবে একটা সতর্কতা — SQLAlchemy-তেও "raw query" চালানোর সুযোগ থাকে (`text()` ফাংশন দিয়ে), আর সেখানে যদি কেউ আবার স্ট্রিং জোড়া লাগিয়ে ফেলে, একই দুর্বলতা ফিরে আসে:

```python
from sqlalchemy import text

# এটাও বিপজ্জনক, SQLAlchemy ব্যবহার করলেও
username = request_body["username"]
db.execute(text(f"SELECT * FROM users WHERE username = '{username}'"))

# সঠিক পদ্ধতি — bind parameter সহ
