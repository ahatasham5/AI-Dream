...
XSS প্রতিরোধের মূল কৌশলটা একটা সহজ নীতিতে দাঁড় করানো যায় — **আউটপুট encode করো, প্রসঙ্গ (context) অনুযায়ী**। মানে, ইউজারের ইনপুট যখন HTML-এর ভেতরে বসছে, তখন সেই ডেটার ভেতরের বিশেষ অক্ষরগুলো (`<`, `>`, `"`, `'`, `&`) তাদের "নিরাপদ" রূপে (HTML entity) রূপান্তর করে দিতে হবে, যাতে ব্রাউজার সেগুলোকে কোড না ভেবে নিছক টেক্সট হিসেবে দেখায়। এই জায়গায় একটা গুরুত্বপূর্ণ, সূক্ষ্ম পয়েন্ট আছে যেটা FastAPI-এর মতো JSON-only API ব্যাকএন্ডে কাজ করার সময় স্পষ্টভাবে বোঝা দরকার।

যদি backend নিজে HTML render করতো (যেমন কোনো template engine দিয়ে — Jinja2, EJS, Handlebars), তাহলে সেই engine ডিফল্টভাবেই আউটপুট escape করে দিতো, আর XSS-এর ঝুঁকিটা মূলত backend-এর নিয়ন্ত্রণেই থাকতো। কিন্তু FastAPI যখন একটা pure JSON API হিসেবে কাজ করে (যেমন `return {"comment": comment.text}`), তখন Pydantic বা FastAPI নিজে থেকে কোনো HTML escaping করে না — এবং করারও কথা না, কারণ JSON রেসপন্সের ভেতরের স্ট্রিং তো HTML না, এটা নিছক ডেটা। `{"comment": "<script>alert(1)</script>"}` একটা সম্পূর্ণ বৈধ, নিরাপদ JSON — সমস্যাটা তখনই শুরু হয় যখন এই ডেটা কোনো frontend (React, Vue, বা সরাসরি `innerHTML`) সেটাকে বিনা sanitize-এ HTML-এর ভেতর বসিয়ে দেয়। তার মানে API-only আর্কিটেকচারে XSS-এর আসল ঝুঁকিটা থাকে **consumer-এর দিকে** — যে frontend সেই JSON-কে কীভাবে render করছে তার উপর, backend-এর escaping-এর উপর না।

তাহলে প্রশ্ন আসে — backend-এর তাহলে কোনো দায়িত্বই নেই? আসলে আছে, দুই জায়গায়। প্রথমত, defense-in-depth নীতিতে backend কখনো frontend-এর sanitization-এর উপর সম্পূর্ণ ভরসা করে বসে থাকতে পারে না, কারণ একই API একাধিক ক্লায়েন্ট (mobile app, তৃতীয় পক্ষের integration, ভবিষ্যতে অন্য কোনো frontend) থেকে ব্যবহৃত হতে পারে, আর প্রতিটার sanitization ঠিক আছে কিনা backend জানে না। দ্বিতীয়ত, যদি সেই একই backend কখনো কোনো HTML সরাসরি জেনারেট করে ফেরায় (যেমন email template, PDF রিপোর্ট, অথবা server-side rendered কোনো পেজ), তখন backend নিজেই এক্সপোজড।

তাই ইউজারের ইনপুট যেখানে সত্যিকারের rich-text HTML হওয়ার কথা (যেমন একটা blog editor থেকে আসা কনটেন্ট), সেখানে সংরক্ষণের আগেই একটা **HTML sanitizer** লাইব্রেরি দিয়ে বিপজ্জনক ট্যাগ ছেঁটে ফেলা উচিত, শুধু frontend-এর উপর নির্ভর না করে:
...
async def create_comment(payload: CommentCreate, current_user=Depends(get_current_user)):
    ...
    # এখন clean_text নিরাপদে ডেটাবেজে সংরক্ষণ করা যায়
    await save_comment(text=clean_text, user_id=current_user.sub)
    return {"message": "কমেন্ট সংরক্ষিত হয়েছে"}
```

সাধারণ প্লেইন-টেক্সট ফিল্ডের ক্ষেত্রে (যেখানে কোনো HTML-এরই দরকার নেই, যেমন username বা comment যদি plain text হওয়ার কথা), সবচেয়ে নিরাপদ পদ্ধতি হলো `<` আর `>`-এর মতো অক্ষর সম্পূর্ণ প্রত্যাখ্যান বা escape করে দেওয়া, রেন্ডারের সময় frontend-এর উপর নির্ভর না করে backend থেকেই Pydantic validator দিয়ে ভ্যালিডেশন বসিয়ে দেওয়া (লেসন ৭-এ input validation নিয়ে বিস্তারিত)। এটাকেই মনে রাখার সহজ নিয়ম বলা যায় — **stored XSS প্রতিরোধ backend-এর দায়িত্ব (sanitize করে সংরক্ষণ), reflected/DOM XSS প্রতিরোধ মূলত frontend-এর দায়িত্ব (render করার সময় escape), কিন্তু API ডকুমেন্টেশনে স্পষ্ট করে বলে দেওয়া উচিত কোন ফিল্ডে raw HTML আসতে পারে, যাতে frontend টিম কখনো ভুলবশত `dangerouslySetInnerHTML`-এর মতো কিছু দিয়ে un-sanitized ডেটা render না করে**।
...
এই header ম্যানুয়ালি লেখা সম্ভব, কিন্তু লেসন ৬-এ আমরা দেখবো `secure` লাইব্রেরি কীভাবে এটা আরও সহজে, গঠনগতভাবে সঠিকভাবে বসাতে সাহায্য করে।
