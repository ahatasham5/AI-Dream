...
async def get_habit(habit_id: str, db: Session = Depends(get_db)):
    result = db.execute(text(f"SELECT * FROM habits WHERE id = '{habit_id}'"))
    return result.fetchone()
```

একটা ভালো AI reviewer সাথে সাথে চিহ্নিত করবে — এখানে raw string দিয়ে SQL query বানানো হচ্ছে, যেটা SQL injection-এর দরজা খুলে দেয়। সঠিক ফিক্স, parameterized query দিয়ে (বা এমনকি ভালো — SQLAlchemy ORM query দিয়ে):

```python
@router.get("/{habit_id}")
async def get_habit(habit_id: uuid.UUID, db: Session = Depends(get_db)):
    result = db.execute(text("SELECT * FROM habits WHERE id = :id"), {"id": str(habit_id)})
    return result.fetchone()
```
