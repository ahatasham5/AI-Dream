...
```python
# Python থেকে Consul-এ query করা — python-consul2 লাইব্রেরি, HTTP API দিয়ে
import consul

client = consul.Consul()

def discover_notification_service():
    _, services = client.health.service("notification-service", passing=True)  # শুধু স্বাস্থ্যবান ইনস্ট্যান্স
    return [
...
async def register_service():
    lease = client.lease(10)  # ১০ সেকেন্ডের lease
    client.put(
        "services/notification-service/instance-1",
        json.dumps({"address": "10.0.2.5", "port": 4002}),
        lease=lease,
    )
...
async def discover_service():
    instances = client.get_prefix("services/notification-service/")
    return [json.loads(value) for value, _ in instances]
```
