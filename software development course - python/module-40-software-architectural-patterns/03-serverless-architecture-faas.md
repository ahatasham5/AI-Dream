...
**প্রোডাকশন নুয়ান্স:** যদি ইতিমধ্যে একটা FastAPI অ্যাপ থাকে আর সেটাকেই Lambda-তে চালাতে চাও, পুরো হ্যান্ডলার নতুন করে লিখতে হয় না — `Mangum` লাইব্রেরি ব্যবহার করে বিদ্যমান FastAPI `app`-কে Lambda handler-এ রূপান্তর করা যায়:
...
@app.get("/silence-check")
async def check_silence(video_url: str):
    silent_ranges = process_video(video_url)
    return {"silent_segments": silent_ranges}

handler = Mangum(app)  # AWS Lambda এখন সরাসরি এই handler-কে কল করবে
```
