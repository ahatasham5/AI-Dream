...
flowchart TD
    ...
    D["সামঞ্জস্যপূর্ণ ফরম্যাটিং - Black/Ruff"] --> Clean
...
def calc(t, d):
    return len([x for x in t if x.d < d])

# ভালো: নাম নিজেই ব্যাখ্যা করছে
def count_overdue_tasks(tasks, current_date):
    return len([task for task in tasks if task.deadline < current_date])
```
