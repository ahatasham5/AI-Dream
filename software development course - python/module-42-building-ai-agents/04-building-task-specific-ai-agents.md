...
```python
refund_tools = [check_order_eligibility_tool, initiate_refund_tool]  # মাত্র দুটো টুল, ইচ্ছাকৃতভাবে


async def run_refund_agent(user_message: str, order_context: dict):
    agent = create_tool_calling_agent(
        llm=model,
        tools=refund_tools,
        prompt=refund_agent_prompt,
    )
    executor = AgentExecutor(agent=agent, tools=refund_tools, max_iterations=3)
    return await executor.ainvoke({"user_message": user_message, **order_context})
```

`max_iterations=3` একটা গুরুত্বপূর্ণ সেফটি প্যারামিটার — এটা এজেন্টকে সর্বোচ্চ তিনবার চিন্তা-টুল কল করার চক্রে সীমাবদ্ধ রাখে। এটা ছাড়া একটা এজেন্ট তাত্ত্বিকভাবে অসীম লুপে আটকে যেতে পারে (LLM বারবার ভুল টুল কল করতে থাকলে), যা সরাসরি টাকার খরচ বাড়ায় (প্রতিটা LLM কলের একটা মূল্য আছে) আর ইউজারকে অপেক্ষায় রাখে।
...
structured_model = model.with_structured_output(RefundDecisionSchema)
```

এই প্যাটার্নটা Module 13-এ শেখা টাইপ-অ্যানোটেশনের রানটাইম-সংস্করণ ভাবা যায় (একই `pydantic` যা FastAPI রিকোয়েস্ট/রেসপন্স মডেলেও ব্যবহার করেছি) — আমরা LLM-কে বাধ্য করছি একটা নির্দিষ্ট আকারে উত্তর দিতে, যাতে আমাদের কোড নিশ্চিতভাবে জানে `approved` একটা বুলিয়ান হবে, `refund_amount` একটা সংখ্যা।
