...
async def handle_support_request(user_message: str, context: dict):
    category = await route_message(user_message)

    match category:
        case "billing":
            return await run_billing_agent(user_message, context)
        case "technical":
            return await run_tech_agent(user_message, context)
        case _:
            return await escalate_to_human(user_message, context)
```
...
```python
async def run_billing_agent(user_message: str, context: dict):
    result = await billing_executor.ainvoke({"user_message": user_message, **context})

    if "NEEDS_TECHNICAL_HELP" in result["output"]:
        return await run_tech_agent(
            user_message,
            {**context, "billing_notes": result["output"]},
        )

    return result["output"]
```

লক্ষ্য করো, `billing_notes` দিয়ে বিলিং এজেন্ট যা বুঝেছে তা টেকনিক্যাল এজেন্টের কাছে হস্তান্তর করা হচ্ছে — ঠিক যেমন Module 50-এর মাইক্রোসার্ভিস আর্কিটেকচারে একটা সার্ভিস ইভেন্টের মাধ্যমে আরেকটা সার্ভিসকে প্রেক্ষাপট পাঠায়, যাতে কাজ আবার শূন্য থেকে শুরু করতে না হয়।
