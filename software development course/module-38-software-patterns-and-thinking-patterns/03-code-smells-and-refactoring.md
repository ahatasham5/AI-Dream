...
function processTask(task) {
  if (!task.title || task.title.length > 100) throw new Error('invalid title');
  if (task.priority === 'high' && !task.deadline) throw new Error('deadline needed');
  const daysLeft = Math.ceil((new Date(task.deadline) - new Date()) / 86400000);
  if (daysLeft < 2) {
    sendEmail(task.assignee, `জরুরি: ${task.title} সময় প্রায় শেষ!`);
  }
...
const URGENT_THRESHOLD_DAYS = 2; // magic number-এর বদলে নামকরণ করা constant

function notifyIfUrgent(task, daysLeft) {
  if (daysLeft < URGENT_THRESHOLD_DAYS) {
    sendEmail(task.assignee, `জরুরি: ${task.title} সময় প্রায় শেষ!`);
  }
}
