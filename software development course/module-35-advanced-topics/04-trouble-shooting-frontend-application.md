...
fetch('/api/habits')
  .then(res => res.json())
  .then(data => {
    // ধরো backend পাঠাচ্ছে { habits: [...] } কিন্তু frontend আশা করছে সরাসরি array
    setHabits(data.map(h => h.title)); // এখানেই error হবে যদি data একটা object হয়
  });
```

এই কোড ভাঙবে কারণ `data` আসলে `{ habits: [...] }` — একটা object, array না। Console-এ এসে দেখাবে `data.map is not a function`। এই ভুলটা backend-এ না, frontend-এ — API contract সম্পর্কে ভুল ধারণার কারণে হয়েছে। এই ধরনের সমস্যা এড়াতে API response-এর গঠন নিয়ে frontend আর backend টিমের মধ্যে স্পষ্ট চুক্তি (contract) থাকা দরকার, আর ডকুমেন্টেশন (Module ৩৬.১২-তে আমরা AI দিয়ে ডকুমেন্টেশন বানানো শিখবো) সেই চুক্তি বজায় রাখতে সাহায্য করে।

আরেকটা সাধারণ frontend সমস্যা হলো **CORS error** — যখন frontend আর backend আলাদা ডোমেইনে চলে আর backend সঠিক header পাঠায় না। এটা দেখতে backend সমস্যার মতো মনে হলেও ঠিক করতে হয় backend-এর CORS middleware ঠিক করে।
