...
class User {
  constructor(name, email, password) {
    this.name = name;
    this.email = email;
    this.password = password; // বাস্তবে পাসওয়ার্ড কখনো plain text-এ রাখা হয় না, hash করে রাখা হয়
    this.isLoggedIn = false;
  }

  login() {
    this.isLoggedIn = true;
    console.log(`${this.name} লগইন করেছে।`);
  }

  logout() {
    this.isLoggedIn = false;
    console.log(`${this.name} লগআউট করেছে।`);
  }
}

const arman = new User("আরমান", "arman@example.com", "hashed_password_123");
...
class Product {
  ...
  sell(quantity) {
    if (quantity > this.stock) {
      console.log("পর্যাপ্ত স্টক নেই!");
      return;
    }
    this.stock -= quantity;
    console.log(`${quantity} টি ${this.title} বিক্রি হলো। বাকি স্টক: ${this.stock}`);
  }
}

const shoe = new Product("রানিং শু", 1500, 20);
