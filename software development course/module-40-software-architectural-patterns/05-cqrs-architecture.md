...
```javascript
// Command Handler — শুধু ডেটা লেখে, স্বাভাবিক normalized টেবিলে
app.post('/tasks', async (req, res) => {
  const task = await TaskWriteModel.create(req.body);
  await publishEvent('task.created', task); // read model-কে জানানো
...
app.get('/dashboard/task-summary', async (req, res) => {
  const summary = await TaskSummaryReadModel.findOne({ userId: req.user.id });
