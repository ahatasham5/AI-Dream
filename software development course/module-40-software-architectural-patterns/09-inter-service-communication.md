...
async function createTask(taskData) {
  ...
  await axios.post('http://notification-service:4002/notify', {
    userId: task.userId,
    message: `নতুন Task তৈরি হয়েছে: ${task.title}`,
  });
...
async function createTask(taskData) {
  const task = await db.tasks.create(taskData);
  await messageQueue.publish('task.created', {
    userId: task.userId,
    title: task.title,
  });
...
messageQueue.subscribe('task.created', async (event) => {
  await sendEmail(event.userId, `নতুন Task: ${event.title}`);
});
```
