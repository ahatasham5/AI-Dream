...
এই `.proto` ফাইল থেকে কোড জেনারেট করা হয়, যাতে সার্ভার আর ক্লায়েন্ট দুই পাশেই টাইপ-সেফ ফাংশন তৈরি হয় — অনেকটা Module ১৩-এ শেখা TypeScript interface-এর মতোই একটা "কন্ট্রাক্ট প্রথম" চিন্তাভাবনা, কিন্তু এটা ভাষা-নিরপেক্ষ (Node.js সার্ভিস, Go সার্ভিস, Python সার্ভিস — সবাই একই `.proto` থেকে কাজ করতে পারে)।

```javascript
// Node.js gRPC ক্লায়েন্ট — Notification Service, Task Service-কে কল করছে
const client = new TaskServiceClient('task-service:50051', grpc.credentials.createInsecure());

client.getTask({ taskId: '123' }, (err, response) => {
  console.log(response.title); // স্ট্রংলি-টাইপড রেসপন্স
});
```
...
পরের লেসনে আমরা Circuit Breaker Pattern-এর গভীরে যাবো — বাস্তব লাইব্রেরি (`opossum`) দিয়ে বাস্তবায়ন, আর স্টেট ট্রানজিশনের আরও বিস্তারিত আলোচনা।
