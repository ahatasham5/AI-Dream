...
```javascript
// Tenant-resolving middleware — JWT থেকে tenant শনাক্ত করা (Module ১২-এর JWT)
...
// প্রতিটা মডেল ফাংশনে tenantId বাধ্যতামূলক প্যারামিটার হিসেবে রাখা
class TaskModel {
  static async findAll(tenantId) {
    return db.query('SELECT * FROM tasks WHERE tenant_id = $1', [tenantId]);
  }

  static async create(tenantId, data) {
    return db.query(
      'INSERT INTO tasks (tenant_id, title) VALUES ($1, $2) RETURNING *',
      [tenantId, data.title]
    );
  }
}

// কন্ট্রোলার — কখনো tenantId ভুলে না যাওয়ার নিশ্চয়তা
app.get('/api/tasks', resolveTenant, async (req, res) => {
  const tasks = await TaskModel.findAll(req.tenantId); // req.user.id নয়, req.tenantId!
  res.json(tasks);
});
```
