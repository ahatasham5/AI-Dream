...
একটা এজেন্টকে সাধারণ Express API-এর মতোই ডেপ্লয় করা যায়, কিন্তু কয়েকটা বিশেষ বিবেচনা যোগ হয়। প্রথমত, LLM কল সাধারণ ডাটাবেজ কোয়েরির চেয়ে অনেক ধীর (কয়েক সেকেন্ড লাগতে পারে), তাই **timeout** আর **streaming response** ভালোভাবে হ্যান্ডল করা জরুরি।
...
app.post('/api/agent/chat', async (req, res) => {
  ...
  try {
    ...
    const result = await Promise.race([
      runAgentWithMemory(userId, message),
      timeoutPromise,
    ]);

    res.json({ reply: result.output });
  } catch (error) {
    console.error('Agent deployment error:', error.message);
    res.status(504).json({ error: 'এজেন্ট সাড়া দিতে দেরি করছে, আবার চেষ্টা করুন' });
  }
});
```
...
const agentLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 10, // প্রতি মিনিটে সর্বোচ্চ ১০টা এজেন্ট কল প্রতি ইউজার
  message: 'অনেক বেশি অনুরোধ, একটু অপেক্ষা করুন',
});
...
```js
function shouldUseNewAgentVersion(userId) {
  const hash = simpleHash(userId) % 100;
  return hash < 5; // মাত্র ৫% ইউজার নতুন ভার্সন পাবে
}
```
