...
HTTP status code ঠিক এই কাজটাই করে ব্যাকএন্ড আর ফ্রন্টএন্ডের মধ্যে। যখন ফ্রন্টএন্ড (বা যেকোনো ক্লায়েন্ট — ব্রাউজার, মোবাইল অ্যাপ, অন্য কোনো সার্ভার) একটা রিকোয়েস্ট পাঠায়, ব্যাকএন্ড শুধু ডেটা ফেরত পাঠায় না — সাথে একটা তিন-অঙ্কের সংখ্যা পাঠায়, যেটা এক নজরে বলে দেয় কাজটা কেমন হলো। Module 2-তে আমরা `200` আর `500` কোড দুটো সংক্ষেপে দেখেছিলাম, যখন আমাদের নিজের বানানো Node.js সার্ভার HTML ফাইল পাঠাচ্ছিলো। এখন সময় হয়েছে পুরো পরিবারটা নিয়ে বিস্তারিত জানার, কারণ একটা প্রফেশনাল API বানাতে গেলে সঠিক status code বেছে নেয়াটা একটা মৌলিক দক্ষতা।
...
Express.js-এ status code পাঠানো খুবই সহজ — `res.status()` মেথড ব্যবহার করে:
...
লক্ষ্য করো, `res.status(404)` লেখার পর সাথে সাথে `.json(...)` চেইন করে বসানো হয়েছে — Express এটাকে সহজভাবে একসাথে করতে দেয়। যদি `res.status()` না লেখো, Express ডিফল্টভাবে `200` পাঠিয়ে দেবে, এমনকি যদি তুমি একটা এরর মেসেজও পাঠাও! তাই ভুল status code পাঠানো একটা খুব সাধারণ কিন্তু বিপজ্জনক ভুল — কারণ ফ্রন্টএন্ড ডেভেলপার status code দেখেই সিদ্ধান্ত নেয় ব্যর্থতা দেখাবে নাকি সফলতা।

status code শেখা মানে হলো তোমার API-কে একটা "সৎ ওয়েটার" বানানো — যে সবসময় সঠিকভাবে জানায় কী ঘটেছে। এই ভিত্তির উপর দাঁড়িয়ে এখন আমরা দেখবো কীভাবে Express-এ একাধিক রুট সুন্দরভাবে সাজানো যায়, যাতে আমাদের API শুধু সৎ না, গোছানোও হয়।
