...
export const updateProductSafely = catchAsync(async (req, res) => {
  const { version, ...updates } = req.body;

  const product = await Product.findOneAndUpdate(
    { _id: req.params.id, version }, // শুধু তখনই আপডেট হবে যদি version মেলে
    { $set: updates, $inc: { version: 1 } },
    { new: true },
  );

  if (!product) {
    throw new AppError(409, 'ডেটা ইতিমধ্যে অন্য কেউ বদলে ফেলেছে, আবার লোড করে চেষ্টা করো');
  }
