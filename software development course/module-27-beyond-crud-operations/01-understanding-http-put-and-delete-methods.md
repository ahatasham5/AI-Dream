...
Express.js-এ এই মেথডগুলো রাউট করা Module 7-এ শেখা রাউটার সিস্টেমেরই সম্প্রসারণ:

```typescript
// product/product.routes.ts
router.get('/products/:id', getProduct);       // Read
router.post('/products', createProduct);        // Create
router.put('/products/:id', replaceProduct);     // Update (সম্পূর্ণ প্রতিস্থাপন)
router.patch('/products/:id', updateProduct);    // Update (আংশিক)
router.delete('/products/:id', deleteProduct);   // Delete
```

লক্ষ্য করো এখানে PATCH-ও যোগ করা হয়েছে, যদিও প্রশ্নটা ছিলো শুধু PUT আর DELETE নিয়ে — কারণ বাস্তবে PUT আর PATCH প্রায়ই একসাথে আলোচনা করতে হয়, তাদের মধ্যেকার সূক্ষ্ম পার্থক্যটা বোঝা জরুরি। DELETE মেথডে সাধারণত রিকোয়েস্ট বডি লাগে না, শুধু URL-এর `:id` প্যারামিটারই যথেষ্ট — কারণ ডিলিট করার জন্য শুধু "কোনটা" জানা দরকার, "কী দিয়ে বদলাবে" জানার দরকার নেই।

```typescript
export const deleteProduct = catchAsync(async (req, res) => {
  const deleted = await Product.findByIdAndDelete(req.params.id);
  if (!deleted) throw new AppError(404, 'প্রোডাক্ট পাওয়া যায়নি');
  res.status(204).send(); // 204 No Content — সফল, কিন্তু ফেরত দেয়ার কিছু নেই
});
```

`204 No Content` স্ট্যাটাস কোডটা লক্ষ্য করার মতো — Module 6-এ স্ট্যাটাস কোড শেখার সময় এটা হয়তো তেমন গুরুত্ব পায়নি, কিন্তু DELETE অপারেশনের জন্য এটাই প্রচলিত সঠিক কোড, কারণ ডিলিট সফল হলে ফেরত দেয়ার মতো নতুন কোনো ডেটা থাকে না।

এখন আমরা জানি PUT আর DELETE কেন আলাদা, কীভাবে সেমান্টিক্যালি ভিন্ন আচরণ করে। পরের লেসনে আমরা Express.js-এ বাস্তবে PUT আর PATCH এন্ডপয়েন্ট ইমপ্লিমেন্ট করবো, আর দেখবো কীভাবে দুটোর লজিক আলাদা লিখতে হয়।
